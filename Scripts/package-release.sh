...
INSTALLER_PKG="$DIST_DIR/SignalLanes-$VERSION-macos-installer.pkg"

rm -rf "$DIST_DIR"
mkdir -p "$DIST_DIR"

SIGNALLANES_VERSION="$VERSION" SIGNALLANES_BUILD="$BUILD_NUMBER" "$ROOT_DIR/Scripts/build-installer.sh" "$VERSION"
